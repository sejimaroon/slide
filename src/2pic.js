...
const App = () => {
  ...
  const handleDownload = async () => {
    ...
    await ffmpeg.run(
      ...
      '-t', `${autoplayDelay}`,            
      '-i', 'input_2.jpg',
      '-filter_complex', `[0]settb=AVTB[v0];[1]settb=AVTB[v1];[v0][v1]xfade=transition=fade:duration=${pageSpeed}:offset=${autoplayDelay - pageSpeed};[v01][v2]xfade=transition=fade:duration=${pageSpeed}:offset=${autoplayDelay - pageSpeed},scale=trunc(iw/2)*2:trunc(ih/2)*2[v]`,          
      '-map', '[v]',       
      '-c:v', 'libx264',
      '-pix_fmt', 'yuv420p',
      '-s', '1340x670',
